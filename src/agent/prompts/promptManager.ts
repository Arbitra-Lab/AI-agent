...
export class PromptManager {
  ...
  public static initialize(): void {
    ...
    logger.info(
      `PromptManager initialized with system prompt v${metadata.version}`,
    );
  }
   ...
   */
  private static registerPromptVersion(
    versionId: string,
    metadata: PromptMetadata,
  ): void {
  ...
  public static getSystemPrompt(context?: PromptContext): RenderedPrompt {
    ...
    if (contextTypes) {
      logger.info(
        `System prompt v${rendered.versionId} rendered with context: ${contextTypes}`,
      );
    }
  ...
  public static logLLMCallInitiation(
    ...
    if (!versionEntry) {
      logger.warn(
        `LLM call references unknown prompt version: ${promptVersionId}`,
      );
    }
