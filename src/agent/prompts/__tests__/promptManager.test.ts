...
describe('PromptManager', () => {
    ...
    describe('LLM Call Logging', () => {
        ...
        test('logs LLM call initiation with version tracking', () => {
            ...
            // Should not throw
            expect(() => {
                PromptManager.logLLMCallInitiation(conversationId, promptVersionId, userMessage, {
                    toolsAvailable: 3,
                });
            }).not.toThrow();
        });
        ...
        test('logs LLM call completion with token usage', () => {
            ...
            // Should not throw
            expect(() => {
                PromptManager.logLLMCallCompletion(conversationId, promptVersionId, response, tokensUsed, {
        ...
        test('warns when logging with unknown version', () => {
            ...
            expect(() => {
                PromptManager.logLLMCallInitiation(conversationId, 'unknown-version', 'test message');
            }).not.toThrow();
        });
    });
    ...
    describe('Version Traceability', () => {
        ...
        test('enables tracing LLM call to specific prompt version', () => {
            ...
            PromptManager.logLLMCallInitiation(conversationId, promptVersionId, 'user query');
            ...
            expect(promptVersionId).toBe(PromptManager.getCurrentSystemPromptVersion());
        });
    });
