...
export class DrizzleEscrowStateRepository implements EscrowStateRepository {
  ...
  async listTracked(): Promise<LocalEscrowRecord[]> {
    const rows = await this.db
      .select({ id: escrows.id, contractId: escrows.contractId, state: escrows.state })
      .from(escrows)
      .where(and(isNotNull(escrows.contractId), notInArray(escrows.state, TERMINAL_STATES)));

    return rows
      .filter((row): row is typeof row & { contractId: string } => row.contractId !== null)
      .map((row) => ({ id: row.id, chainEscrowId: row.contractId, state: row.state }));
  }
